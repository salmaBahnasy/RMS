...
interface SubscriptionsAndMembershipsProps {
  employeeAffiliatedAuthority?: string | number | undefined;
  membershipExpiryGregorian?: string | number;
}


const SubscriptionsAndMemberships: React.FC<SubscriptionsAndMembershipsProps> = ({employeeAffiliatedAuthority, membershipExpiryGregorian}) => {
  ...
  return (
    <View style={{ marginHorizontal: 20, marginBottom: 20, backgroundColor: COLORS.white, borderRadius: SIZES.radius, padding: 16 }}>
      <View style={{ ...styles.row, justifyContent: 'space-between' } as StyleProp<ViewStyle>}>
        <Text style={FONTS.h2 as StyleProp<TextStyle>}>{t('subscriptionsAndMemberships')}</Text>
       
      </View>
      <View> 
        <ListViewItem fstTitle={t('employeeAffiliatedAuthority')} fstValue= {employeeAffiliatedAuthority}/>
        <ListViewItem fstTitle={t('membershipExpiryGregorian')} fstValue= {membershipExpiryGregorian} />
      </View>
    </View>
  );
};

export default SubscriptionsAndMemberships;